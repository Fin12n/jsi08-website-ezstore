...
function resolveMockValue(currentVal, fieldVal) {
  if (fieldVal && typeof fieldVal === 'object' && fieldVal._type === 'arrayUnion') {
    const list = Array.isArray(currentVal) ? [...currentVal] : [];
    fieldVal.values.forEach(v => {
      // JSON stringify checks for object equality in case of versions array
      const exists = list.some(x => JSON.stringify(x) === JSON.stringify(v));
      if (!exists) list.push(v);
    });
    return list;
  }
  if (fieldVal && typeof fieldVal === 'object' && fieldVal._type === 'arrayRemove') {
    const list = Array.isArray(currentVal) ? [...currentVal] : [];
    return list.filter(v => !fieldVal.values.some(x => JSON.stringify(x) === JSON.stringify(v)));
  }
  return fieldVal;
}
        ...
        return {
          ...
          set: async (data, options = {}) => {
            ...
            for (const [k, v] of Object.entries(data)) {
              resolvedData[k] = resolveMockValue(idx !== -1 ? list[idx][k] : undefined, v);
            }
            if (idx !== -1) {
              if (options.merge) {
                list[idx] = { ...list[idx], ...resolvedData, id: docId };
              } else {
                list[idx] = { id: docId, ...resolvedData };
              }
            } else {
              list.push({ id: docId, ...resolvedData });
            }
            return true;
          },
          delete: async () => {
          ...
          update: async (data) => {
            ...
            if (idx !== -1) {
              const resolvedData = {};
              for (const [k, v] of Object.entries(data)) {
                resolvedData[k] = resolveMockValue(list[idx][k], v);
              }
              list[idx] = { ...list[idx], ...resolvedData, id: docId };
            } else {
              const resolvedData = {};
              for (const [k, v] of Object.entries(data)) {
                resolvedData[k] = resolveMockValue(undefined, v);
              }
              list.push({ id: docId, ...resolvedData });
            }
            return true;
...
const dbWrapper = {
  ...
  FieldValue: {
    arrayUnion: (...args) => {
      if (realDb) return FieldValue.arrayUnion(...args);
      return { _type: 'arrayUnion', values: args };
    },
    arrayRemove: (...args) => {
      if (realDb) return FieldValue.arrayRemove(...args);
      return { _type: 'arrayRemove', values: args };
    }
  },
  collection: (collectionName) => {
    return {
