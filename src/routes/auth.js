...
const { sendVerificationEmail, sendResetPasswordEmail, isMock: isEmailMock } = require('../config/email');
...
router.post('/forgot-password', isGuest, async (req, res) => {
  ...
  try {
    ...
    if (emailCheck.empty) {
      return res.render('forgot-password', {
        title: 'Quên mật khẩu - EZ Studio',
        error: 'Không tìm thấy tài khoản nào liên kết với email này.'
      });
    }

    const userDoc = emailCheck.docs[0];
    ...
    // Generate reset URL
    const resetUrl = `${req.protocol}://${req.get('host')}/reset-password?token=${token}&email=${encodeURIComponent(email.trim().toLowerCase())}`;

    // Send reset password email
    await sendResetPasswordEmail(email.trim().toLowerCase(), resetUrl);

    res.render('forgot-password', {
      title: 'Quên mật khẩu - EZ Studio',
      success: `Một liên kết khôi phục mật khẩu đã được gửi đến email ${email}!`
    });
  } catch (error) {
    console.error('❌ Forgot password database error:', error);
    res.render('forgot-password', {
      title: 'Quên mật khẩu - EZ Studio',
      error: 'Hệ thống gặp sự cố khi xử lý yêu cầu.'
    });
  }
});
...
router.get('/reset-password', isGuest, async (req, res) => {
  const { token, email } = req.query;

  if (!token || !email) {
    return res.redirect('/forgot-password?error=Yêu cầu không hợp lệ. Vui lòng thử lại!');
  }
  ...
  try {
    ...
    if (snapshot.empty) {
      return res.redirect('/forgot-password?error=Không tìm thấy tài khoản với email này.');
    }

    const userData = snapshot.docs[0].data();

    // Verify token and expiry
    if (userData.resetPasswordToken !== token || !userData.resetPasswordExpires || Date.now() > userData.resetPasswordExpires) {
      return res.redirect('/forgot-password?error=Liên kết khôi phục mật khẩu không hợp lệ hoặc đã hết hạn.');
    }

    res.render('reset-password', {
      title: 'Đặt lại mật khẩu - EZ Studio',
      token,
    ...
    res.redirect('/forgot-password?error=Hệ thống gặp sự cố. Vui lòng thử lại!');
  }
});
...
router.post('/reset-password', isGuest, async (req, res) => {
  ...
  if (!token || !email || !password || !confirmPassword) {
    return res.redirect('/forgot-password?error=Thông tin khôi phục mật khẩu bị thiếu.');
  }
  ...
  try {
    ...
    if (snapshot.empty) {
      return res.redirect('/forgot-password?error=Tài khoản không tồn tại.');
    }
    ...
    console.log(`✔ Password reset successfully for user: ${userData.username}`);
    res.redirect('/login?success=Đặt lại mật khẩu thành công! Bạn có thể đăng nhập bằng mật khẩu mới.');
  } catch (error) {
