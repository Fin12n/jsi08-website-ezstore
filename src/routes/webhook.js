...
router.post('/sepay', async (req, res) => {
  ...
  try {
    ...
    // 4. Check description matching
    const orderMatch = transferContent.match(/(EZS|ORD|PAY)\s*([a-zA-Z0-9\-]+)/i);
    const napMatch = transferContent.match(/NAP\s+([a-zA-Z0-9]+)/i);

    if (orderMatch) {
      const orderId = orderMatch[2].trim();
      console.log(`Sepay Webhook: Processing direct bank payment for Order ID: ${orderId}`);
      ...
      if (orderData.status === 'completed') {
        console.log(`Đơn hàng ${orderId} đã được hoàn thành trước đó.`);
      ...
      // Record a transaction for Buyer
      await db.collection('transactions').add({
        userId: orderData.userId,
        type: 'TOPUP',
        amount: orderData.totalPrice,
        status: 'COMPLETED',
        description: `Nạp tiền qua Chuyển khoản ngân hàng trực tiếp cho đơn hàng #${orderId} (SePay: ${referenceCode}, Số tiền: ${transferAmount?.toLocaleString('vi-VN')}đ)`,
    ...
    if (!napMatch) {
      console.log('Nội dung chuyển khoản không hợp lệ (không khớp NAP hoặc EZS):', transferContent);
      return res.status(200).json({ success: true, message: 'Ignored - invalid content format' });
    }

    const extractedCode = napMatch[1].toUpperCase();
